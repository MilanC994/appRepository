      ...
      <React.Fragment>
        <div>
          <table style={{ margin: "auto" }}>
            <tbody>
              {this.props.coinProp.map((coin) => (
                <CoinContainer
                  key={coin.id}
                  disable={this.props.btnDisabled}
                  id={coin.id}
                  value={coin.value}
                  decrementProp={this.props.decrementProp}
                  incrementProp={this.props.incrementProp}
                  count={coin.count}
                />
              ))}
            </tbody>
          ...
          <button
            ...
            onClick={() => this.props.setToPay()}
          >
            <h2> Apply:</h2>
          </button>

          <Pay
            toPay={this.props.getToPay}
            disable={this.props.dsblPay}
            calculate={this.props.calculate}
            setPayed={this.props.setPayed}
            calculate={this.props.calculate}
          />
          <span className="badge m-2 badge-secondary">
            <h5>{this.props.getOutputString}</h5>
          </span>
        </div>
      </React.Fragment>
    );
